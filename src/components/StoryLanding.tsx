...
function ChapterHead({
  ...
  return (
    <div className="scroll-chapter-head" data-reveal>
      <p className="scroll-chapter-label">{chapter}</p>
      <SplitTitle line1={line1} line2={line2} />
...
export function StoryLanding({ onExplore, onSimulate }: Props) {
  ...
  return (
    <div className="scroll-book is-unlocked" ref={rootRef}>
      ...
      {/* 01 COSTS */}
      <section className="scroll-chapter" id="costs">
        <ChapterHead
        ...
        />
        <div className="scroll-cost-table" data-reveal>
          {SCROLL_COSTS.rows.map((r) => (
            <article key={r.year} className="scroll-cost-row">
        ...
        <h3 className="scroll-subhead" data-reveal>
          {SCROLL_COSTS.breakdown.title}
        </h3>
        <div className="scroll-machine-grid">
          {SCROLL_COSTS.breakdown.items.map((m, i) => (
            <article key={m.title} className="scroll-machine-card" data-reveal>
        ...
        <p className="scroll-foot" data-reveal>
          {SCROLL_COSTS.foot}
        </p>
      </section>

      {/* 02 REVENUE */}
      <section className="scroll-chapter" id="revenue">
        <ChapterHead
        ...
        />
        <div className="scroll-rev-block" data-reveal>
          ...
          <div className="scroll-stat-grid">
            {SCROLL_REVENUE.fifa.items.map((item) => (
              <article key={item.label} className="scroll-stat">
        ...
        <div className="scroll-rev-block" data-reveal>
          ...
          <div className="scroll-machine-grid">
            {SCROLL_REVENUE.host.items.map((m, i) => (
              <article key={m.title} className="scroll-machine-card">
                <span className="scroll-machine-num">0{i + 1}</span>
                <h3>{m.title}</h3>
                <p>{m.body}</p>
              </article>
      ...
      {/* 03 PROFIT */}
      <section className="scroll-chapter" id="profit">
        <ChapterHead
        ...
        <div className="scroll-gap-grid">
          {SCROLL_PROFIT.gaps.map((g) => (
            <article key={g.name} className="scroll-gap" data-reveal>
              <h3>{g.name}</h3>
        ...
        <div className="scroll-lesson-grid" style={{ marginTop: "2rem" }}>
          {SCROLL_PROFIT.points.map((p, i) => (
            <article key={p.title} className="scroll-lesson" data-reveal>
              <span>0{i + 1}</span>
              <h3>{p.title}</h3>
              <p>{p.body}</p>
            </article>
        ...
        <div className="scroll-dual" data-reveal style={{ marginTop: "2.5rem" }}>
          <article className="scroll-dual-pane fifa">
            <span className="scroll-dual-year">{SCROLL_PROFIT.dual.left.year}</span>
          ...
          <article className="scroll-dual-pane host">
            <span className="scroll-dual-year">{SCROLL_PROFIT.dual.right.year}</span>
      ...
      {/* 04 ACHIEVEMENTS */}
      <section className="scroll-chapter" id="achieve">
        <ChapterHead
        ...
        <div className="scroll-lesson-grid">
          {SCROLL_ACHIEVE.items.map((item) => (
            <article key={item.index} className="scroll-lesson" data-reveal>
      ...
      {/* 05 BEFORE / AFTER */}
      <section className="scroll-chapter" id="before-after">
        <ChapterHead
        ...
        <div className="scroll-ba-list">
          {SCROLL_BEFORE_AFTER.cases.map((c) => (
            <article key={c.name} className="scroll-ba" data-reveal>
      ...
      {/* 06 RISKS */}
      <section className="scroll-chapter" id="risks">
        <ChapterHead
      ...
      {/* 07 HARMS */}
      <section className="scroll-chapter" id="harms">
        <ChapterHead
        ...
        <div className="scroll-lesson-grid">
          {SCROLL_HARMS.items.map((item) => (
            <article key={item.index} className="scroll-lesson harm" data-reveal>
              <span>{item.index}</span>
              <h3>{item.title}</h3>
              <p>{item.body}</p>
            </article>
      ...
      <section className="scroll-close">
        <blockquote className="scroll-close-quote" data-reveal>
          <p>“{SCROLL_CLOSE.quote}”</p>
        </blockquote>
        <div className="scroll-close-cta" data-reveal>
          <h2 className="scroll-title">
